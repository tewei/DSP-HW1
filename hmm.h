...
typedef struct{
   ...
   double pi[MAX_STATE];			//initial prob.
   double a[MAX_STATE][MAX_STATE];	//transition prob.
   double b[MAX_OBSERV][MAX_STATE];	//observation prob.
} HMM;
...
static void dumpHMM( FILE *fp, HMM *hmm )
{
   ...
   for( i = 0 ; i < hmm->state_num - 1; i++ )
      fprintf( fp, "%.5lf ", hmm->pi[i]);
   fprintf(fp, "%.5lf\n", hmm->pi[ hmm->state_num - 1 ] );

   fprintf( fp, "\ntransition: %d\n", hmm->state_num );
   for( i = 0 ; i < hmm->state_num ; i++ ){
      for( j = 0 ; j < hmm->state_num - 1 ; j++ )
         fprintf( fp, "%.5lf ", hmm->a[i][j] );
      fprintf(fp,"%.5lf\n", hmm->a[i][hmm->state_num - 1]);
   }

   fprintf( fp, "\nobservation: %d\n", hmm->observ_num );
   for( i = 0 ; i < hmm->observ_num ; i++ ){
      for( j = 0 ; j < hmm->state_num - 1 ; j++ )
         fprintf( fp, "%.5lf ", hmm->b[i][j] );
      fprintf(fp,"%.5lf\n", hmm->b[i][hmm->state_num - 1]);
   }
}
